...
class EnsembleForensicClassifier:
    ...
    def analyze_audio(self, dsp_results, audio_path=None):
        ...
        # 1. Spatial Reverb Net: Spoof risk depends on room reverberation anomalies (RT60 > 0.6s or low C50 clarity)
        reverb_risk = max(0.0, min(1.0, (rt60 - 0.5) * 1.5 + max(0.0, (10.0 - c50) * 0.08)))
        ...
        # 4. Spatial Acoustics GMM: Spoof risk correlates with overall RIR and spectral anomalies
        scores["spatial_gmm"] = float(out_gmm[1]) * 0.05 + 0.95 * (reverb_risk * 0.4 + breathing_risk * 0.6)
